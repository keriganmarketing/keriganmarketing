...
add_shortcode( 'whitepaper_form', function($atts){
    ...
    if($_POST['sec'] == '' && $_POST['formId'] == 'Whitepaper Download' ){
        ...
        if($passCheck) {
            ...
            $adminintrocopy = '<p ' . $copystyle . '>Details are below:</p>';
            $receiptintrocopy = '<p ' . $copystyle . '>Thank you for your interest in our whitepaper, ' . $whitepaper['name'] . '. <a href="' . $whitepaper['link'] . '" >Click here to download</a>.</p>';
	        $dateofemail = '<p style="font-size:12px; ' . $fontstyle . ' color:#000; text-align:center; margin:20px 0 0;">Date Submitted: ' . date('M j, Y') . ' @ ' . date('g:i a') . '</p>';
            ...
            $receiptdata = array(
                'headline' => '<h2 ' . $headlinestyle . '>' . $sendreceipt['subject'] . '</h2>',
                'introcopy' => $receiptintrocopy . $followups . $dateofemail,
            );
            ...
            $leads->wp_insert_post(
                array( //POST INFO
                    ...
                    'meta_input' => array( //POST META
                        ...
                        'lead_info_email_address' => $email,
                        'lead_info_interests' => $rServiceList,
                        'lead_info_message' => 'Whitepaper Download: ' . $website . ', ' . $whitepaper['name'],
                    )
                ), true
            );

            $successmessage .= '<strong>Thank you for downloading our whitepaper, ' . $whitepaper['name'] . '. You may <a href="' . $whitepaper['name'] . '" >Download it here</a>. We\'ve emailed you a download link as well.</strong>';
            $showAlert = '<div class="alert alert-success digital-marketing" role="alert">'.$successmessage.'</div>';
